...
#### Backlog cleanup (one-time sweep, filing only — no task/draft proposals)
All scopes exclude `-label:Triaged/Backlog` so runs are idempotent. Process oldest-first, cap 100/run, apply both the category label AND `Triaged/Backlog`. End with per-label counts + "Reply 'continue' for next batch, 'stop' to end."

- `cleanup status` — counts for: total in:inbox, unread, already-Triaged/Backlog, age buckets (<7d, 7-30d, 30-90d, 90d+).
- `cleanup last <N>d` — scope: `in:inbox newer_than:<N>d`.
- `cleanup sender <email-or-domain>` — scope: `in:inbox from:<arg>`.
- `cleanup unread older than <N>d` — scope: `in:inbox is:unread older_than:<N>d`.
- `continue` — re-run last scope (next ≤100).
- `stop` — halt.

Extra rules: never exceed 100/run. Actionable mail still goes to `Inbox/Action` but do NOT auto-propose tasks/drafts during cleanup. If scope is empty, say so and exit.
...
### Output format — `cleanup`
Same shape as `triage` (per-label counts), plus a header line with scope + remaining count, and the "Reply 'continue' / 'stop'" footer. Skip task/draft lines.
